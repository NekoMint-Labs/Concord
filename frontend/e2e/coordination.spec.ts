...
async function workspace(request: APIRequestContext): Promise<Workspace> {
  const response = await request.get(`${project}/workspace`, {
...
async function event(request: APIRequestContext, change = { revision: "V17" }) {
  const response = await request.post(`${project}/events`, {
    headers: authorization,
    data: {
      id: randomUUID(),
      ...
      title: "Browser E2E design update",
      change,
    },
  });
...
test.beforeEach(async ({ request, page }) => {
  ...
  await expect
    .poll(async () => {
      ...
      return (
        current.run?.id === resetRun.id &&
        current.run.status === "COMPLETED" &&
        !current.stale &&
        current.analysis?.readiness.every((row) => row.status === "READY")
      );
    })
    .toBe(true);
  await page.addInitScript(() => {
    if (!sessionStorage.getItem("cca-token"))
...
test("coordinates a design change through evidence, approval, receipt, and a fresh recheck", async ({
  ...
  const denied = await request.post(`/api/proposals/${proposal.id}/execute`, {
    headers: authorization,
  });
  ...
  const receipt = await request.get(
    `/api/operations/${proposal.operation_id}`,
    { headers: authorization },
  );
  expect(receipt.status()).toBe(200);
  const firstReceipt = await receipt.json();
  await request.post(`/api/proposals/${proposal.id}/execute`, {
    headers: authorization,
  });
  ...
  expect((await workspace(request)).state.version).toBe(
    firstReceipt.after_version,
  );
...
test("stale approval is rejected and the browser refreshes instead of silently approving", async ({
  page,
  request,
}) => {
  const panel = inspector(page);
  ...
  const old = staleWorkspace.proposals.find(
    (item) => item.work_package_id === "WP-200",
  )!;
  ...
  await page.route(workspaceRoute, (route) =>
    route.fulfill({ json: staleWorkspace }),
  );
  await event(request, { revision: "V18" });
  const rejected = page.waitForResponse((response) =>
    response.url().endsWith(`/proposals/${old.id}/approve`),
  );
  await approve.click();
  ...
  expect(
    (await workspace(request)).approvals.some(
      (approval) => approval.proposal_id === old.id,
    ),
  ).toBe(false);
});

test("inspection R4 requires exact typed confirmation", async ({
  page,
  request,
}) => {
  const response = await request.post(`${project}/events`, {
...
test("an outdated judgement is raised beside the work package, not as a global banner", async ({
  page,
  request,
}) => {
...
test("viewer can read but cannot approve or execute", async ({
  page,
  request,
}) => {
  await event(request);
  await page.evaluate(() =>
    sessionStorage.setItem("cca-token", "local-demo-viewer"),
  );
  await page.reload();
  await selectDemoPackage(page);
  await expectBlocked(page);
...
test("the change composer supports keyboard selection and dismissal in the product control", async ({
  ...
  await expect(
    page.getByRole("option", { name: "班组人员不足" }),
  ).toBeFocused();
  await page.keyboard.press("Enter");
...
test("a disconnected event stream refreshes stale approvals behind a newer document job", async ({
  page,
  request,
}) => {
  let attempts = 0;
  ...
  await expect
    .poll(async () => {
      ...
      return (
        !refreshed.stale && refreshed.analysis?.id !== current.analysis?.id
      );
    })
    .toBe(true);
  const refreshed = await workspace(request);
  ...
  expect(
    refreshed.approvals.some((item) => item.proposal_id === proposal.id),
  ).toBe(false);
  const freshProposal = refreshed.proposals.find(
    (item) => item.work_package_id === "WP-200",
  )!;
  ...
  const approved = await request.post(
    `/api/proposals/${freshProposal.id}/approve`,
    {
      headers: authorization,
      data: { strong: false, confirmation: "" },
    },
  ...
  await expect(
    panel.getByRole("button", { name: "已批准", exact: true }),
  ).toBeVisible();
  await expect(
    panel.getByRole("button", { name: "执行并重新检查" }),
  ).toBeEnabled();

...
**Master/detail row grammar**: the sidebar work-package rows, the Documents source list, and the BIM element list share one treatment - an `--accent-muted` selected surface at `--radius-sm`, a 13px title that goes 400 → 600 when selected, and 11px secondary metadata, all on a square row. Selection is learned once and applies everywhere. *(This paragraph previously described a 2px left rule; that was replaced by the tinted surface, which carries area where a 2px marker in a 46px row does not.)* ~~Source and element lists sit on `--surface-detail`~~ *(superseded: they sit on `--surface-list` - see "Blocker-fix pass" below)*; the reading surface stays on `--surface-workspace`; evidence chunks and element properties are separated by rules rather than boxed into cards.

## Blocker-fix pass: plane separation, the shell's own height, and pane intent

The rendered product was audited in a Windows review and in a real browser, and it disagreed with three claims this document had made. The corrections are recorded here because each one is a *relationship*, not a value.

**The plane ladder was re-spaced, and `--surface-list` joined it.** The ladder had been declared as five planes ordered by recession with the local object browser spent as a half step *inside* it. Rendered, that produced a local browser 1.035:1 from the chrome bands that name it and 1.051:1 from the Inspector's plane - invisible differences - and `--sidebar` had been pointed at `--surface-chrome`, so global navigation, every pane header, both toolbars, the timeline and the local browser were all one value. Measured, the sidebar and the local source list were 1.035:1 apart. The ladder is now `--bg-app` `#cbcdcf` → `--surface-nav` `#d6d7d8` → `--surface-detail` `#dedfe0` → `--surface-list` `#e7e8e9` → `--surface-chrome` `#f0f1f2` → `--surface-workspace` `#fbfbfc`, `--surface-list` is part of the ordered ladder the contract tests, and `--sidebar` resolves to `--surface-nav` again. `--surface-chrome` and `--surface-workspace` did not move: that pair already read correctly. The control-boundary tokens moved with the plane they are reserved for (`--line-control` `#777879`, `--line-control-soft` `#7d7e7f`), because `--line-control` is only just clear of the 3:1 floor on the navigation plane and has no room of its own.

**The shell's height was not owned.** `.application-shell` was `100dvh` and did fill the window, but the library gives a pane a height and leaves it `display: block`, so `.sidebar` and `.main-shell` - block children with no height of their own - were content-height. At 1440x900 the sidebar painted 349px and the work plane 472px, and the shell's own background showed for the remaining 428px. Both shell panes are now marked `pane-stack`, which is the product's existing name for "this pane lays its children out as a column", and a filling child is a growing child.

**A pane declared in pixels keeps those pixels.** `Pane` now sets `groupResizeBehavior="preserve-pixel-size"` for every pane whose `defaultSize` is a pixel value. The library default carries a *percentage* across a window resize, so a temporary clamp at a narrow window was re-normalized into the layout and then restored as the clamped ratio when the window widened again - the navigation column ended up parked at its own 320px ceiling. The percentage ceilings on the local browsers and the Inspector are also stated in pixels now, for the same reason: a percentage ceiling shrinks with the window and can bind a width the user chose by hand.
